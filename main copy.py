...
def dadosIniciais():
    ...
    foundSelectorVideo = dir_path/"videos"
...
for videoType in postar:
    ...
    data_atual = (date.today()).day # Utilizado quando o horário do envio muda de um dia p outro
    ...
    for videoNumber in range(start, end+1):
        ...
        time.sleep(5)
        openNavConfig()
        success = steps.step1Upload()
        if success == False:
            continue
        success = steps.step2Edit()
        if success == False:
            continue
