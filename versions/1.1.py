...
def aboutVideoInfos():
    global videoType, foundSelector, tittle, desc
    
    foundSelector = rf"C:\Users\Luis Carlos\Desktop\FCBerlotti\ytUpload\videos\{videoType}"

    try:
        with open(f"tittle/{videoType}.txt", "r", encoding="utf-8") as file:
            tittle = file.read()
    except FileNotFoundError:
        print(f"Erro: O arquivo 'desc/{videoType}.txt' não foi encontrado.")
    except Exception as e:
        print(f"Erro ao ler o arquivo: {str(e)}")
    
    try:
        with open(f"desc/{videoType}.txt", "r", encoding="utf-8") as arquivo:
            desc = arquivo.read()
    except FileNotFoundError:
        print(f"Erro: O arquivo 'desc/{videoType}.txt' não foi encontrado.")
    except Exception as e:
...
# Horario para postagem de videos
"""
Hour: 11:00 / 15:00 / 19:00
"""

videoType = 'roblox' # Escolher um tema das categorias disponiveis
start = 2
end = 6
jumpDay = 7
firstDate = 8
